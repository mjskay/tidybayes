...
test_that("[add_]linpred_rvars works on a simple rstanarm model", {
  ...
  expect_equal(add_linpred_rvars(mtcars_tbl, m_hp_wt, n = 10, seed = 1234), filtered_ref)
})
...
test_that("[add_]linpred_rvars works on brms models with dpar", {
  ...
  expect_equal(add_linpred_rvars(mtcars_tbl, m_hp_sigma, n = 10, seed = 1234, dpar = TRUE), filtered_ref)
})
...
test_that("[add_]linpred_rvars throws an error when nsamples is called instead of n in brms", {
  skip_if_not_installed("brms")
  m_hp = readRDS(test_path("../models/models.brms.m_hp.rds"))

  expect_error(
    m_hp %>% linpred_rvars(newdata = mtcars_tbl, nsamples = 100),
    "`nsamples.*.`n`.*.See the documentation for additional details."
  )
  expect_error(
    m_hp %>% add_linpred_rvars(newdata = mtcars_tbl, nsamples = 100),
    "`nsamples.*.`n`.*.See the documentation for additional details."
  )
})
...
test_that("[add_]linpred_rvars throws an error when re.form is called instead of re_formula in rstanarm", {
  ...
  expect_error(
    m_hp_wt %>% linpred_rvars(newdata = mtcars_tbl, re.form = NULL),
    "`re.form.*.`re_formula`.*.See the documentation for additional details."
  )
  expect_error(
    m_hp_wt %>% add_linpred_rvars(newdata = mtcars_tbl, re.form = NULL),
    "`re.form.*.`re_formula`.*.See the documentation for additional details."
  )
})
