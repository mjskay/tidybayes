...
test_that("[add_]predicted_rvars and basic arguments works on an rstanarm model with random effects", {
  ...
  expect_equal(predicted_rvars(m_cyl, mtcars_tbl, n = 100, seed = 123), ref)
  expect_equal(add_predicted_rvars(mtcars_tbl, m_cyl, n = 100, seed = 123), ref)
})
...
test_that("[add_]predicted_rvars works on a simple brms model", {
  ...
  expect_equal(predicted_rvars(m_hp, mtcars_tbl, n = 100, seed = 123), ref)
  expect_equal(add_predicted_rvars(mtcars_tbl, m_hp, n = 100, seed = 123), ref)
})
...
test_that("[add_]predicted_rvars works on brms models with categorical outcomes", {
  ...
  expect_equal(predicted_rvars(m_cyl_mpg, mtcars_tbl, seed = 1234, n = 100), ref)
  expect_equal(add_predicted_rvars(mtcars_tbl, m_cyl_mpg, seed = 1234, n = 100), ref)
})
...
test_that("[add_]predicted_rvars works on brms models with dirichlet responses", {
  ...
  expect_equal(predicted_rvars(m_dirich, grid, seed = 1234, n = 100), ref)
})
...
test_that("[add_]predicted_rvars works on brms models with multinomial responses", {
  ...
  expect_equal(predicted_rvars(m_multinom, grid, seed = 1234, n = 10), ref)
  ...
  expect_equal(predicted_rvars(m_multinom, grid, seed = 1234, n = 10, columns_to = "col_pred"), column_ref)
})

test_that("[add_]predicted_rvars throws an error when nsamples is called instead of n in brms", {
  skip_if_not_installed("brms")
  m_hp = readRDS(test_path("../models/models.brms.m_hp.rds"))
...
test_that("[add_]predicted_rvars throws an error when draws is called instead of n in rstanarm", {
  skip_if_not_installed("rstanarm")
  m_hp_wt = readRDS(test_path("../models/models.rstanarm.m_hp_wt.rds"))
