...
Plotting point summaries and with one interval is straightforward using `geom_pointinterval()`:
...
To see the density along with the intervals, we can use `stat_eye()` ("eye plots", which combine intervals with violin plots), or `stat_halfeye()` (interval + density plots):
...
To plot this example, we'll also show the use of `stat_pointinterval()` instead of `geom_pointinterval()`, which summarizes draws into points and intervals within ggplot:
...
Where `add_fitted_draws()` is analogous to `brms::fitted.brmsfit()` (or `brms::posterior_linpred()`), `add_predicted_draws()` is analogous to `brms::predict.brmsfit()` (or `brms::posterior_predict()`), giving draws from the posterior predictive distribution.
...
Let's demonstrate `compare_levels()` with `stat_halfeye()`:
