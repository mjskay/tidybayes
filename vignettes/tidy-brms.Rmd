...
Or, for posterior predictions (instead of means), we can go back to probability bands:
...
Note: for the `.category` variable to reatin the original factor level names you
must be using `brms` greater than version 2.15.
...
fit_plot = mtcars_clean %>%
  ...
  add_epred_draws(m_cyl, epred = "P(cyl | mpg)", category = "cyl") %>%
...
```{r fig.width = med_width, fig.height = med_height}
tibble(mpg = 20) %>%
  add_epred_draws(m_cyl, epred = "P(cyl | mpg = 20)", category = "cyl") %>%
  ungroup() %>%
...
data_plot_with_mean = mtcars_clean %>%
  ...
  # ONLY do this IF you are planning to make spaghetti plots, etc.
  # NEVER subsample to a small sample to plot intervals, densities, etc.
  add_epred_draws(m_cyl, epred = "P(cyl | mpg)", category = "cyl", ndraws = 100) %>%
  group_by(mpg, .draw) %>%
