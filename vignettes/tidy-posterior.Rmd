...
mtcars %>%
  ...
  # NOTE: this shows the use of n to subsample within add_epred_draws()
...
p = mtcars %>%
  group_by(cyl) %>%
  data_grid(hp = seq_range(hp, n = 101)) %>%
  add_epred_rvars(m_mpg, n = ndraws) %>%
  unnest_rvars() %>%
  ggplot(aes(x = hp, y = mpg, color = ordered(cyl))) +
...
to move the nested columns into rows. This will also add a `.row` column indexing which row of the input
data frame each prediction came from:

```{r}
tibble(mpg = c(21,22)) %>%
  add_epred_rvars(m_cyl, columns_to = "cyl_pred")
```

Note how the `cyl_pred` variable does not retain the original factor level names. This appears to be due to a bug in brms. As a workaround, we can recover the original factor labels and assign the result to a `cyl` column:

```{r}
tibble(mpg = c(21,22)) %>%
  add_epred_rvars(m_cyl, columns_to = "cyl") %>%
  mutate(cyl = levels(mtcars_clean$cyl)[factor(cyl)])
```
...
data_plot_with_mean = mtcars_clean %>%
  data_grid(mpg = seq_range(mpg, n = 101)) %>%
  # NOTE: use of n = 100 here subsets draws for the creation of spaghetti plots;
  # DOT NOT do this if you are making other chart types like intervals or densities
  add_epred_rvars(m_cyl, epred = "P(cyl | mpg)", n = 100) %>%
...
subtracting `beta[1]` from the linear predictor and from the `beta[2]`
as these values are all highly correlated (thus would be hard to visualize with
uncertainty in a meaningful way without looking at their differences). We'll
also demonstrate the use of `.width = ppoints(XXX)` where `XXX` is a number
...
esoph %>%
  ...
  mutate(tobgp = levels(esoph$tobgp)[factor(tobgp)]) %>%
...
esoph_plot +
  geom_col(aes(x = median(.epred)), fill = "gray75", width = 1, color = "white") +
