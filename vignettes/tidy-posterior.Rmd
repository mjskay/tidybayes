...
mtcars %>%
  ...
  # this shows the use of n to subsample within add_epred_draws()
...
mtcars %>%
  ...
  add_predicted_rvars(m_mpg) %>%
  ggplot(aes(x = hp)) +
  stat_dist_lineribbon(aes(dist = .prediction), .width = c(.99, .95, .8, .5), color = brewer.pal(5, "Blues")[[5]]) +
  geom_point(aes(y = mpg), data = mtcars) +
  scale_fill_brewer() +
...
tibble(mpg = c(21,22)) %>%
  ...
  mutate(cyl = `P(cyl | mpg)` %**% rbind(4,6,8))
```
