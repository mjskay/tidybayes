...
So let's try half-eye plots instead. The basic idea is that we need to get the three parameters for the sampling distribution of each parameter and then use `stat_dist_halfeye()` to plot them. The sampling distribution for parameter $i$, $\beta_i$, from an `lm` model is a scaled-and-shifted t distribution:
