...
#'
#' - `add_epred_draws()` adds draws from the **expectation of the posterior predictive
...
#'
#' - `add_linpred_draws()` adds draws from the **posterior linear predictors** to the data.
...
#'     This is returned by `add_linpred_draws(transform = FALSE)` for \pkg{brms} and \pkg{rstanarm} models.
...
#'     This is returned by `add_linpred_draws(transform = TRUE)` for \pkg{brms} and \pkg{rstanarm} models.
#'     It is analogous to `type = "response"` in [predict.glm()].
#'
#'   **NOTE:** `add_linpred_draws(transform = TRUE)` and `add_epred_draws()` may be equivalent but
...
#'   use `add_epred_draws()` if available, and if not available, verify this property holds prior
#'   to using `add_linpred_draws()`.
#'
#' - `add_residual_draws()` adds draws from residuals,
...
#' versus `predicted_draws(object, newdata)`. This facilitates use in data
