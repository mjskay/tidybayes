...
* __Visualizing priors and posteriors__. The focus on tidy data makes the output from tidybayes
  ...
  `stat_pointinterval()`), visualizing
  distributions with point summaries and intervals (the `stat_sample_slabinterval()` family of
  stats, including eye plots, half-eye plots, CCDF bar plots, gradient plots, dotplots, and histograms), 
  and visualizing fit lines with an arbitrary number of uncertainty bands (`geom_lineribbon()` 
  and `stat_lineribbon()`). Priors can also be visualized in the same way using the 
  `stat_dist_slabinterval()` family of stats. The `geom_dotsinterval()` family also
