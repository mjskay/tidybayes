...
[tidybayes](http://mjskay.github.io/tidybayes) is an R package that aims to make it easy to integrate popular Bayesian 
...
* __Summarizing posterior distributions__ from models. The `ggdist::point_interval()`
  ...
  and they **respect data frame groups**. These functions 
...
* __Visualizing priors and posteriors__. The focus on tidy data makes the output from tidybayes
  ...
  `geom_linerange`) can give useful output, but `tidybayes` (via `ggdist`) also includes several
  geoms and stats to simplify common combinations of `stats` and `geoms` with sensible
  ...
  `ggdist::stat_dist_slabinterval()` family of stats. The `ggdist::geom_dotsinterval()` family also
...
Automatic splitting of indices into columns makes it easy to plot the condition means here. We will employ the `tidybayes::stat_eye()` geom, which combines a violin plot of the posterior density, median, 66% and 95% quantile interval to give an "eye plot" of the posterior. The point and interval types are customizable using the `point_interval()` family of functions. A "half-eye" plot (non-mirrored density) is also available as `tidybayes::stat_halfeye()`. All tidybayes geometries automatically detect their appropriate orientation, though this can be overridden with the `orientation` parameter if the detection fails.
